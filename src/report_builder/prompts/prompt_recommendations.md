# TETR.IO 戦績レポート 別紙AI分析HTML 生成プロンプト

> 使い方：「===プロンプト本文===」以降をすべてコピーし、AI連携用の集計JSON
> （例: `reports/*_external_ai_payload.json`）だけを添付して、任意のAIチャットに渡してください（コード実行不要）。
> 完成HTML・Base64画像・CSV/Parquet等の元データ・グラフPNGは添付しません。
> 本体レポートは定量分析までを担当し、本プロンプトは独立した別紙HTMLとして「総合まとめと定性分析」を作成します。
...
<div class="wrap">
  ...
  <footer>この別紙はAI連携用集計JSONから作成した定性分析です。本体レポートの数値定義と集計結果を前提にしています。</footer>
</div>
</body>
</html>
```
