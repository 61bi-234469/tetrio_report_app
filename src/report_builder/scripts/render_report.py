...
def render_chapters(bundle: AnalysisBundle, project_root: Path) -> None:
    ...
    c1 += block(
        "横軸はJSTのマッチ日時、縦軸はTR・Est. TRです。配置直後の欠損区間は線から除外しています。",
    ...
    c2 = chapter_header(2, "成長推移と安定性", f"マッチ日時ベースの指標推移と、初期{s['growth_window_n']}マッチ・直近{s['growth_window_n']}マッチの変化を確認します。")
    ...
    c3 = chapter_header(3, "能力バランス", "主要指標の分布と、相手平均に対する能力バランスを確認します。レーダーとスタイルは直近100マッチをマッチ単位で集計します。")
    ...
    c4 += block(
        "横軸は自分VS−相手VSを分位ビンに分けた平均、縦軸はそのビンの勝率です。0より右は相手よりVSが高いマッチです。",
    ...
    c4 += block(
        "列がAPM優劣、行がVS優劣です。各セルは勝率とマッチ数を示します。",
        ...
        "同じセルでもTR差・マッチ時間・相手スタイルが異なるため、セル間差は純粋な因果ではありません。APMの優位がVSを含む相対優位に結びついているかをセル別に確認します。",
        ("高", "hi"),
    )
    ...
    c6 = chapter_header(6, "接戦・決着局面", "接戦になったマッチをどう閉じたかを確認します。タイブレークはマッチ単位、スコア状況別の次ラウンド勝率と最終ラウンドの能力変化はラウンド単位で扱います。")
    ss_rows = [
    ...
    c7 = chapter_header(7, "ラウンド展開とマッチ時間", "ラウンドが長短どの帯に入ったとき何が起きたかを確認します。決着時間分布・ラウンド時間別勝率・時間帯別の能力差分をいずれもラウンド単位で扱います。")
    ...
    c8 = chapter_header(8, "連戦の流れとセッション内のマッチ位置", "マッチとマッチの間で続く流れを確認します。連勝連敗、前マッチ結果、セッション内のマッチ位置、TRドローダウンをマッチ単位で集計します。")
...
<p class="lead">元parquetから正規化して再計算し、単マッチ・単ラウンド・期間窓を区別しています。最短ラウンドは切断・DQ等のアーティファクト疑いを避けるため5秒未満を除外します。単ラウンドPRや初期配置期のTR変動、派生指標の外れ値は、注意欄を確認してください。</p>
{records_table}
</details>
