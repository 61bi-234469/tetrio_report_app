...
定量レポートとは別に、外部の生成AIへ素材一式を渡して作る、定性的な分析レポートです。深掘りの考察や言語化を外部AIに任せたい場合に使います。ツールが次の3つの素材を書き出します。

1. **AI分析用プロンプト** — 分析手順を指示するプロンプト（`src/report_builder/prompts/`。GUI出力時は `reports/<player>_external_ai_prompt.md`）。
2. **軽量な集計JSON** — 集計済みの数値だけを含む軽量データ（CSV・CSS・画像は含みません。GUI出力時は `reports/<player>_external_ai_payload.json`）。
3. **HTMLテンプレート** — AIが自己完結HTMLとして出力するための雛形（`src/report_builder/template`）。

この3点をコード実行（Python）が使える外部AIに渡すと、AIが数値を踏まえて別紙の自己完結HTMLレポートを生成します。
...
②定性分析レポート用の素材（軽量な集計JSON）も書き出す場合は `-PrepareAI` を付けます。プロンプトは `src/report_builder/prompts/`、HTMLテンプレートは `src/report_builder/template` にあります。
...
- API取得データ: `data`
- ①定量レポート（HTML）: `reports`
- ②定性分析レポート用の素材（AIプロンプト・軽量JSON）: `reports`（GUI）/ `src/report_builder/output`（コマンド `-PrepareAI`）
- ②定性分析レポート用のHTMLテンプレート: `src/report_builder/template`
