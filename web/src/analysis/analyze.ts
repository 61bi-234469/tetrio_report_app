...
// python版②レポートの「見るリプレイ（条件）」に合わせた、ラウンド単位の静的条件。
...
const ROUND_REPLAY_CONDITIONS: Array<{
...
}> = [
  {
    ...
    priority: () => "高",
    test: (r) => r.opponentMp && !r.roundWon,
  ...
  },
  {
    ...
    priority: () => "高",
    test: (r) => r.ownMp && !r.opponentMp && !r.roundWon,
  ...
  },
  {
    ...
    detail: (r) =>
      r.metricLabel && r.metricValue !== null ? `${r.metricLabel}${signedNumber(r.metricValue, r.metricDigits)}` : "",
  },
  {
    key: "metric_high",
...
function buildReplayCandidates(matches: MatchRow[], rounds: RoundRow[]): Array<Record<string, unknown>> {
  const metricKeys = [
    ["APM差", "apm", "opponent_apm", 1],
    ["PPS差", "pps", "opponent_pps", 2],
    ["VS差", "vs", "opponent_vs", 1],
    ["Area差", "Area", "opponent_Area", 1],
  ] as const;
  const scalers = new Map<string, (value: unknown) => number | null>();
  for (const [, own, opponent] of metricKeys) {
    scalers.set(own, robustZFactory(rounds.map((round) => {
      const ownValue = toNumber(round[own]);
      const opponentValue = toNumber(round[opponent]);
      return ownValue !== null && opponentValue !== null ? ownValue - opponentValue : null;
    })));
  ...
  for (const [matchNumber, group] of roundsByMatch.entries()) {
    ...
    for (const round of sorted) {
      ...
      let metricDigits = 1;
      for (const [label, own, opponent, digits] of metricKeys) {
        const ownValue = toNumber(round[own]);
        const opponentValue = toNumber(round[opponent]);
        const delta = ownValue !== null && opponentValue !== null ? ownValue - opponentValue : null;
        const z = scalers.get(own)?.(delta) ?? null;
        if (z === null) {
          continue;
        }
  ...
  for (const match of matches) {
    ...
    if (weakQuadrant && weakQuadrant.predicate(match)) {
      ...
      if (match.won) {
        hits.push({ ...base, key: "style_weak_win", priority: "中", scene: "勝利", label: "相性の悪いスタイルに勝てたマッチ", detail, magnitude: depth });
      } else {
        hits.push({ ...base, key: "style_weak_loss", priority: "高", scene: "敗北", label: "相性の悪いスタイルに落としたマッチ", detail, magnitude: depth });
      }
    }
    ...
    if (expected !== null && !match.won && expected >= 0.6) {
      hits.push({ ...base, key: "upset_loss", priority: "高", scene: "敗北", label: "勝てそうだった相手に落としたマッチ", detail: `マッチ勝ち期待${Math.round(expected * 100)}%`, magnitude: expected });
    }
    if (expected !== null && match.won && expected <= 0.4) {
      hits.push({ ...base, key: "upset_win", priority: "中", scene: "勝利", label: "格上に勝ち切ったマッチ", detail: `マッチ勝ち期待${Math.round(expected * 100)}%`, magnitude: 1 - expected });
    }
  ...
  const ranked = [...hits].sort(compareReplayHits);
