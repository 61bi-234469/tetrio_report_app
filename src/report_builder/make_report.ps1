...
param(
    ...
    # ②AI考察レポートの品質モード。-PrepareAI / -GenerateAIReport で共通です。
    [ValidateSet("standard", "high_quality", "low_cost", "legacy_appendix")]
    [string]$AIQuality = "standard",
...
"@
    ...
    # ②AI考察レポート用の品質別AI用JSON・プロンプト・スキーマを生成する。
    # AIチャット貼り付け手順とAIエージェントCLI手順で共通の素材となる。
    if ($PrepareAI -or $GenerateAIReport) {
        Write-Host "②AI素材を生成します（品質: $AIQuality）"
        Invoke-Checked `
            -FilePath $VenvPython `
            -Arguments @(
        ...
        Write-Host "  使用JSON: $($AiSummaryMap[$AIQuality])"
    }
    ...
    if ($GenerateAIReport) {
        ...
        & $VenvPython (Join-Path $Root "scripts\run_ai_agent_cli.py") `
            "--agent" $AIAgent "--quality" $AIQuality
