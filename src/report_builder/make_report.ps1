...
param(
    ...
    # 併せて②AI考察レポート用のAI用JSON・プロンプト・スキーマを保存します。
    [switch]$PrepareAI,

    # ②AI考察レポートをAIエージェントCLIで自動作成します。
    [switch]$GenerateAIReport,

    # -GenerateAIReport で使う連携AIエージェントCLI。
    [ValidateSet("codex", "claude")]
    [string]$AIAgent = "codex",

    # ②AI考察レポートのAIエージェント推論レベル。入力JSONは単一形式を使います。
    [Alias("AIQuality")]
    [ValidateScript({ $_ -in @("standard", "high", "low", "high_quality", "low_cost") })]
    [string]$AIReasoningLevel = "standard",
    ...
    # ライバル章で対戦相手の実名を表示します（既定は匿名ラベル）。
    [switch]$ShowOpponentNames,
...
if ($AIReasoningLevel -eq "high_quality") {
    $AIReasoningLevel = "high"
}
elseif ($AIReasoningLevel -eq "low_cost") {
...
"@
    ...
    if ($ShowOpponentNames) {
        $UpdateArgs += "--show-opponent-names"
    }
    ...
    $AiPayloadPath = $null
    $AiReportPath = $null
    $AiInputJson = "ai_appendix_data.json"
    ...
    # ②AI考察レポート用の単一JSON・プロンプト・スキーマを生成する。
    # AIReasoningLevel はAIエージェントCLIの推論レベルへ反映する。
    if ($PrepareAI -or $GenerateAIReport) {
        Write-Host "②AI素材を生成します（推論レベル: $AIReasoningLevel）"
        Invoke-Checked `
            -FilePath $VenvPython `
            -Arguments @(
                (Join-Path $Root "scripts\prepare_ai_summary.py"),
                "--reasoning-level", $AIReasoningLevel
            ) `
            -FailureMessage "AIサマリの生成に失敗しました。"
        Invoke-Checked `
            -FilePath $VenvPython `
            -Arguments @(
                (Join-Path $Root "scripts\build_ai_prompt.py"),
                "--reasoning-level", $AIReasoningLevel
            ) `
            -FailureMessage "AIプロンプトの生成に失敗しました。"

        $AiCacheDir = Join-Path $CacheDir "ai"
        Write-Host "②AI素材: $AiCacheDir"
        Write-Host "  使用JSON: $AiInputJson"
    }

    # AIエージェントCLIによる②自動作成。失敗しても①本体HTMLは保持する。
    if ($GenerateAIReport) {
        Write-Host ""
        Write-Host "②AI考察レポートをAIエージェントCLIで自動作成します ($AIAgent)" -ForegroundColor Cyan

        & $VenvPython (Join-Path $Root "scripts\run_ai_agent_cli.py") `
            "--agent" $AIAgent "--reasoning-level" $AIReasoningLevel
        $AiRunExit = $LASTEXITCODE

        if ($AiRunExit -ne 0) {
            Write-Warning "AIエージェントCLIの実行に失敗しました。①本体HTMLは保持されます。"
            Write-Host "AIチャット貼り付け手順へ切り替えられます。素材: $(Join-Path $CacheDir 'ai')"
        }
        else {
            & $VenvPython (Join-Path $Root "scripts\validate_ai_text.py")
            if ($LASTEXITCODE -ne 0) {
                Write-Warning "AI本文JSONの検証に失敗しました。②のみ失敗扱いとし、①本体HTMLは保持されます。"
            }
            else {
                $AiStem = [System.IO.Path]::GetFileNameWithoutExtension($ReportData.output_filename)
                $AiReportPath = Join-Path $OutputDir ("{0}_ai_report.html" -f $AiStem)
                & $VenvPython (Join-Path $Root "scripts\render_ai_text_to_html.py") `
                    "--output" $AiReportPath
                if ($LASTEXITCODE -ne 0) {
                    Write-Warning "②AI考察レポートのレンダリングに失敗しました。①本体HTMLは保持されます。"
                    $AiReportPath = $null
                }
                else {
                    Write-Host "②AI考察レポート: $AiReportPath" -ForegroundColor Green
                }
            }
        }
    }
    ...
    Write-Step 5 5 "完了"
    Write-Host ""
    Write-Host "① 戦績レポート（本体）HTML:" -ForegroundColor Green
    Write-Host $ReportPath
    if ($null -ne $AiReportPath) {
        Write-Host ""
        Write-Host "② AI考察レポートHTML:" -ForegroundColor Green
        Write-Host $AiReportPath
    }
    Write-Host ""
